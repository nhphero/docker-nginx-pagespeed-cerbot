...
if [ -z ${NGINX_PAGESPEED+x} ]; then
    ...
    if [ "$NGINX_PAGESPEED" == "on" ]; then
        sed -i "/pagespeed off;/cpagespeed on;" /etc/nginx/default.d/pagespeed.conf
        sed -i "/pagespeed off;/cpagespeed on;" /etc/nginx/include/pagespeed.conf
    else
        sed -i "/pagespeed on;/cpagespeed off;" /etc/nginx/default.d/pagespeed.conf
...
if [ "$NGINX_PAGESPEED_IMG" == "on" ]; then
    sed -i "s/DisableFilters/EnableFilters/" /etc/nginx/default.d/pagespeed-image.conf
else
    sed -i "s/EnableFilters/DisableFilters/" /etc/nginx/default.d/pagespeed-image.conf
fi
...
if [ "$NGINX_PAGESPEED_JS" == "on" ]; then
    sed -i "s/DisableFilters/EnableFilters/" /etc/nginx/default.d/pagespeed-js.conf
else
    sed -i "s/EnableFilters/DisableFilters/" /etc/nginx/default.d/pagespeed-js.conf
fi
...
if [ "$NGINX_PAGESPEED_CSS" == "on" ]; then
    sed -i "s/DisableFilters/EnableFilters/" /etc/nginx/default.d/pagespeed-css.conf
else
    sed -i "s/EnableFilters/DisableFilters/" /etc/nginx/default.d/pagespeed-css.conf
fi
...
if [ -z ${NGINX_PAGESPEED_STORAGE+x} ]; then
    ...
    if [ "$NGINX_PAGESPEED_STORAGE" == "redis" ]; then
        if [ -z ${NGINX_PAGESPEED_REDIS+x} ]; then
            ...
            rm -f /etc/nginx/default.d/pagespeed-redis.conf
        else
            echo "env: NGINX_PAGESPEED_REDIS: [ ${NGINX_PAGESPEED_REDIS} ]"
            printf "# redis storage backend\n" 								>  /etc/nginx/default.d/pagespeed-redis.conf
    ...
    if [ "$NGINX_PAGESPEED_STORAGE" == "memcached" ]; then
        if [ -z ${NGINX_PAGESPEED_MEMCACHED+x} ]; then
            ...
            rm -f /etc/nginx/default.d/pagespeed-memcached.conf
        else
            echo "env: NGINX_PAGESPEED_MEMCACHED: [ ${NGINX_PAGESPEED_MEMCACHED} ]"
            printf "# memcached storage backend\n" 									>  /etc/nginx/default.d/pagespeed-memcached.conf
